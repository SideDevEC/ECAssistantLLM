...
**Addendum 2026-09-26 pm2 (batch restructure A–E — deterministic shared decode step):** The per-token decode loop moved from each session into the coordinator. `StepAsync(BatchSession)`: sessions register for a step generation (first registrant drives); under the cycle gate the driver calls each ready session's `InternalSampleAndFeed()` (samples with the session's OWN grammar + per-request `ActiveSampling` ?? config-driven session defaults, feeds `conv.PromptTokens([tok])` TOKEN-NATIVE — the old TokenToPiece→EnqueuePrompt(text)→retokenize round-trip is gone), then ONE `InferAll` for all fed tokens — decode batch width = number of ready sessions, deterministically; a lone session steps immediately (batch=1, same latency as before). Ops/mutations UNCHANGED: the FIFO is still the only mutation path (Save/Rewind/Reset/prompts), applied under the cycle gate; the request gate already guaranteed mutations land at generation boundaries in the old design (the per-token drain only ever flushed the session's own feedback). Initial prompts flush via an explicit prefill `RunInferCycleAsync` in `InferCoreAsync` before the token loop — StepAsync never drains ops (E: per-token path bypasses graveyard/snapshot drain; C: version-guarded snapshot cache replaces per-cycle ToList). Regression found+fixed during the work: the first version dropped the prefill (StepAsync didn't flush the ops queue) → stale logits → 422 on tools; the explicit prefill restores prompt→prefill-decode→token-steps. Tests 323/323 (BatchVsStandard 8/8). Follow-up: batch throughput should now match the benchmark's measured 85–89 aggregate shape; concurrent-session E2E perf validation pending.


**Addendum 2026-09-26 (penalties now LIVE + configurable window — engine behavior change):** ECAssistantInference (native) now actually applies repeat/present penalties — previously the engine's penalties ring was never populated and `repeat_penalty: 1.1` in the configs was inert. Mechanism (ECI ARCHITECTURE pm6-pm8): per-sample reset+replay of the conversation's recent-token window into a thread-pooled penalties ring; the ring is rebuilt per call so batched interleaving can never cross-contaminate sessions. Consequences for the LLM: (1) output distribution shifts deliberately — anti-repetition now works (tool-call object loops, repeated sentences get suppressed); (2) new `"repeat_last_n"` inference-config knob (default 64, llama-server parity; -1 = engine auto) flows via `InferenceDefaults` → `SessionRegistry.CreateSamplingConfig`; the static RequestRouter arms pass -1 (engine auto resolves to 64); (3) all four runtime configs (~/.ECALLM batch16k/prod16k/e2e/e2e16k) carry `"repeat_last_n": 64` explicitly; (4) servers must run a Release-build dylib (build-metal-rel — see benchmark README / ECI pm3). Follow-up: tool-call suites + journey validation with the new dylib before production rollout.
